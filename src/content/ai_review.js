...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .gl-ai-spinner { width: 14px; height: 14px; border: 2px solid #e1e4e8; border-top-color: #1f883d; border-radius: 50%; animation: gl-ai-spin 0.9s linear infinite; }
      ...
      .gl-ai-code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size:12px; color:#24292f; background:#f6f8fa; border:1px solid #d0d7de; padding:8px; border-radius:6px; white-space: pre; overflow:auto; }
      @media (prefers-color-scheme: dark) { .gl-ai-code { color:#c9d1d9; background:#0d1117; border-color:#30363d; } }
      ...
      @media (prefers-color-scheme: dark) { .gl-ai-apply { background:#161b22; color:#c9d1d9; border-color:#30363d; } .gl-ai-apply[data-status="0"] { background: #000; color: #fff; border-color: #000; } .gl-ai-apply[data-status="0"]:hover { background: #333; } .gl-ai-apply[data-status="1"] { background: #ccc; color: #000; border-color: #ccc; } }
    `;
  ...
  function findEditButton() {
    ...
    for (const root of roots) {
      for (const sel of prioritized) {
        ...
        if (el && isVisible(el) && !el.closest('.gl-new-dropdown-panel, .dropdown-menu, [data-testid="base-dropdown-menu"]')) return el;
      }
    }
    ...
    for (const root of roots) {
      for (const sel of fallbacks) {
        ...
        for (const n of nodes) {
          ...
          if (text === 'edit' || title === 'edit' || aria === 'edit merge request' || /\/edit$/.test(href)) {
            if (isVisible(n) && !n.closest('.gl-new-dropdown-panel, .dropdown-menu, [data-testid="base-dropdown-menu"]')) return n;
          }
        }
      }
    }
  ...
  function renderAnalysisHtml(analysis) {
    ...
    if (findings.length > 0) {
      ...
      for (let i = 0; i < findings.length; i++) {
        ...
        const remParts = [];
        if (f.remediation && f.remediation.steps) remParts.push(`<div class=\"gl-ai-subtitle\">Remediation</div><pre class=\"gl-ai-code\">${escapeHtml(f.remediation.steps)}</pre>`);
        if (f.remediation && f.remediation.diff) remParts.push(`<div class=\"gl-ai-subtitle\">Suggested Diff</div><pre class=\"gl-ai-code\">${escapeHtml(f.remediation.diff)}</pre>`);
  ...
  function createUi(beforeEl) {
    ...
    try { beforeEl.insertAdjacentElement('beforebegin', wrapper); }
    catch { wrapper.style.position = 'fixed'; wrapper.style.top = '12px'; wrapper.style.right = '12px'; wrapper.style.zIndex = '9999'; document.body.appendChild(wrapper); }
    ...
    let onReposition = null; let resizeObs = null;
    function repositionPanel() {
      const margin = 12, gap = 6; const rect = btn.getBoundingClientRect();
      const pr = panel.getBoundingClientRect(); let panelWidth = pr.width || 420; let panelHeight = pr.height || 200;
      let top = rect.bottom + gap; let left = rect.left;
      if (left + panelWidth > window.innerWidth - margin) left = Math.max(margin, window.innerWidth - margin - panelWidth);
      if (left < margin) left = margin;
      if (top + panelHeight > window.innerHeight - margin) {
        const above = rect.top - gap - panelHeight;
        if (above >= margin) top = above; else { top = Math.max(margin, Math.min(top, window.innerHeight - margin - panelHeight)); panel.style.maxHeight = `calc(100vh - ${2 * margin}px)`; }
      }
      panel.style.top = `${Math.round(top)}px`; panel.style.left = `${Math.round(left)}px`;
    }
    function attachReposition() {
      if (onReposition) return; onReposition = () => { if (panel.classList.contains('visible')) repositionPanel(); };
      window.addEventListener('resize', onReposition, { passive: true });
      window.addEventListener('scroll', onReposition, { passive: true, capture: true });
      if ('ResizeObserver' in window && !resizeObs) { resizeObs = new ResizeObserver(() => { if (panel.classList.contains('visible')) repositionPanel(); }); resizeObs.observe(panel); }
    }
    function detachReposition() {
      if (!onReposition) return; window.removeEventListener('resize', onReposition, { passive: true }); window.removeEventListener('scroll', onReposition, { passive: true, capture: true }); onReposition = null; if (resizeObs) { try { resizeObs.disconnect(); } catch {} resizeObs = null; }
    }
    function showPanel() { panel.classList.add('visible'); panel.style.visibility = 'hidden'; requestAnimationFrame(() => { repositionPanel(); panel.style.visibility = ''; attachReposition(); }); }
    function hidePanel() { panel.classList.remove('visible'); detachReposition(); }

    closeBtn.addEventListener('click', hidePanel);

    contentEl.addEventListener('click', async (e) => {
      const t = e.target; if (!t || !(t instanceof Element)) return; const abtn = t.closest('.gl-ai-apply'); if (!abtn) return; if (abtn.getAttribute('data-status') === '1' || abtn.getAttribute('data-loading') === '1') return; e.preventDefault();
      const findingId = abtn.getAttribute('data-finding-id'); if (!findingId) return; const prev = abtn.textContent; abtn.setAttribute('data-loading', '1'); abtn.textContent = 'Applying…';
      try {
        const resp = await chrome.runtime.sendMessage({ type: 'adopt_change', findingId, pageUrl: location.href });
        if (!resp || !resp.ok) throw new Error((resp && resp.error) || 'Failed'); abtn.textContent = 'Applied'; abtn.setAttribute('data-status', '1');
      } catch (err) {
        abtn.textContent = 'Failed'; setTimeout(() => { abtn.textContent = prev; abtn.removeAttribute('data-loading'); repositionPanel(); }, 1400);
      } finally { abtn.removeAttribute('data-loading'); repositionPanel(); }
    });

    btn.addEventListener('click', async () => {
      ensureStyles(); if (panel.classList.contains('visible')) { hidePanel(); return; }
      showPanel();
      const mrUrl = location.href;
      const loadingSpan = loadingEl.querySelector('span:last-child');
      loadingEl.style.display = 'flex'; loadingSpan.textContent = 'Analyzing…';
      contentEl.style.display = 'none'; contentEl.textContent = '';
      ...
      try {
        ...
        const data = resp.data || {}; const status = data.status; const analysis = data.analysisResult;
        loadingEl.style.display = 'none';
        if (status === 'success') { contentEl.style.display = 'block'; contentEl.innerHTML = renderAnalysisHtml(analysis); }
        else { errorEl.style.display = 'block'; errorEl.textContent = data.errorMessage || 'Analysis failed.'; }
        requestAnimationFrame(() => repositionPanel());
      } catch (e) {
        loadingEl.style.display = 'none'; errorEl.style.display = 'block'; errorEl.textContent = `Request failed: ${e.message || e}`; repositionPanel();
      }
    });
  }

  function ensureUi() { if (!isMrPage()) return; ensureStyles(); const editEl = findEditButton(); if (editEl) createUi(editEl); }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ensureUi); else ensureUi();
  const mo = new MutationObserver(() => { if (isMrPage() && !document.getElementById(BTN_ID)) ensureUi(); });
